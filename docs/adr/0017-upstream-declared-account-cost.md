...
显式上游零倍率表示已确认零成本。服务端成本解析器输出 `confirmed`、`unsupported_billing_scope` 或 `missing_upstream_evidence`；前者必须同时返回数值（包括零），后两者必须返回 `null`。不受支持的非 Token 计费范围与缺少请求时证据的 Token／历史记录不得再合并为模糊的“待核算”，也不得用 1x、0x、当前账号设置或当前探测结果回算。不回填历史记录。
...
- 管理端使用记录可在浏览器本地隐藏全部 Provider Account 成本展示；账号身份、客户扣费和 Excel 完整字段不受该偏好影响。
- 导出未知成本为空并附带原因状态，零成本为零；不把未知写成零。
- 汇总同时返回可信条数、暂不支持条数与缺证据条数，三者之和必须等于总请求数；旧 `unconfirmed_requests` 作为两类未知之和继续兼容。
