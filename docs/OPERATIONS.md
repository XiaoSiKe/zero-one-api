...
`v0.2.0@aa236488351eb71e120fc2b6fb32e36b0374c918`。托管迁移保留了完整 Git
...
- Exercise one dedicated probe API Key after deploy, then verify its hashed Redis `apikey:auth:` entry reports snapshot `version: 22` and carries `long_context_pricing_enabled`, `model_pricing`, both Fast flags and the reasoning over-limit policy. Never print or store the raw API Key or the complete cache document.
