...
内部错误使用发布前七天的规范 taxonomy 作为基线。新 taxonomy 立即失败；
已知 taxonomy 只有在观察期内至少出现 5 次，且折算速率同时超过每小时 10 次
和历史速率 6 倍时才视为突增。单个既有上游不可用签名会记录但不会制造误回滚。
账单写入、重复账单、panic/fatal、容器重启、挂载变化和历史数据指纹变化仍然
立即失败，不受该基线规则放宽。
...
- Exercise one dedicated probe API Key after deploy, then verify its hashed Redis `apikey:auth:` entry reports snapshot `version: 24` and carries `long_context_pricing_enabled`, `model_pricing`, the disabled-by-default `model_allowlist`, both Fast flags, the reasoning over-limit policy and `codex_models_manifest_config`. Never print or store the raw API Key or the complete cache document.
