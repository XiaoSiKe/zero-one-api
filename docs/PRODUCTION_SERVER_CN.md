...
应用回滚使用**本次发布前恢复点**记录的源码、Sub2API/Edge 镜像摘要和环境副本，
按分阶段回滚流程恢复对应容器。历史发布章节中的摘要和迁移条数只描述当次发布，
不得用来替代本次实际记录。默认不回滚数据库；保留已应用迁移，逐文件核对账本，
并使用发布前隔离演练验证过的兼容镜像。只有在隔离验证证明数据库本身损坏、且已
进入维护窗口时，才允许从已验证备份恢复数据库。
