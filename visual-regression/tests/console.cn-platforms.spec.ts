...
test.describe('Recovered CN Provider management contracts', () => {
  ...
  test('non-target routes load only the small router seam', async ({ page }) => {
    ...
    page.on('request', (request) => {
      ...
      if (path.startsWith('/assets/cn-provider-admin-v6/')) adapterAssets.push(path)
    })

    await page.goto(`${consoleOrigin}/admin/dashboard`)
    await expect(page.locator('.app-shell')).toBeVisible()
    await expect.poll(() => adapterAssets).toEqual([
      '/assets/cn-provider-admin-v6/cn-provider-admin.js',
    ])
  ...
  test('Provider catalog leaf load failure keeps the approved shell and retries', async ({ page }) => {
    ...
    let leafRequests = 0
    page.on('pageerror', (error) => pageErrors.push(error.message))
    await page.route('**/assets/cn-provider-admin-v6/cnProviderAdminLeaf-*.js', (route) => {
      leafRequests += 1
      if (leafRequests === 1) {
        return route.fulfill({ status: 503, contentType: 'text/javascript', body: '' })
      }
