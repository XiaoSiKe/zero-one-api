...
test.describe('Console public auth contracts', () => {
  ...
  test('login mounts before stalled settings and optional Console adapters settle', async ({ page }) => {
    ...
    page.on('request', (request) => {
      ...
      if (pathname === '/assets/cn-provider-shell-v9/LoginView-BbpS8aW1.js') {
  ...
  test('registration survives stalled settings and optional Console adapters', async ({ page }) => {
    ...
    await page.route('**/assets/online-image-v15/online-image.js', async (route) => {
      await stalledOnlineAdapter
      await route.fallback().catch(() => {})
    })
    await page.route('**/assets/cn-provider-admin-v6/cn-provider-admin.js', (route) =>
    ...
    page.on('request', (request) => {
      const pathname = new URL(request.url()).pathname
      if (pathname === '/assets/cn-provider-shell-v9/RegisterView-CP_DoJ_R.js') {
  ...
  test('administrator login redirect tolerates deferred navigation adapters', async ({ page }) => {
    ...
    try {
      ...
      await expect.poll(() => adapterRequested).toBe(true)
      await expect.poll(() => headerAdapterVersion).toBe('27')
...
test.describe('Console visual contracts', () => {
  ...
  test('serves the approved recovered Console snapshot', async ({ page }) => {
    ...
    expect(html).toContain('/assets/cn-provider-admin-v6/cn-provider-admin.js')
    expect(html).toContain('/assets/cn-provider-shell-v9/index-9xJBhx8B.js')
    expect(html).toContain('/assets/online-image-v15/online-image.js')
    ...
    expect(html).toContain('/assets/zero-one-header-custom-menu-v1.js?v=27')
    expect(html).not.toContain('/assets/zero-one-header-custom-menu-v1.js?v=26')
