...
for cn_provider_version in cn-provider-admin-v1 cn-provider-admin-v4 cn-provider-admin-v6 cn-provider-admin-v7; do
...
for online_image_version in online-image-v15 online-image-v16; do
	for online_image_asset_path in "$test_dir"/console/assets/$online_image_version/*; do
		...
		online_image_asset_headers=$(curl -fsSI -H "Host: $request_host" "$edge_url/assets/$online_image_version/$online_image_asset")
