...
for console_path in setup home login register email-verify forgot-password reset-password key-usage model-plaza dashboard keys images batch-image usage redeem affiliate available-channels profile subscriptions purchase orders admin monitor; do
	...
	assert_text "$console_variant" 'await import("/assets/cn-provider-shell-v10/index-9xJBhx8B.js")' "Console route /$console_path escaped the recovered snapshot"
	...
	assert_text "$console_slash_variant" 'await import("/assets/cn-provider-shell-v10/index-9xJBhx8B.js")' "canonical Console route /$console_path/ escaped the recovered snapshot"
done
...
cn_provider_shell_headers=$(curl -fsSI -H "Host: $request_host" "$edge_url/assets/cn-provider-shell-v10/index-9xJBhx8B.js")
...
cn_provider_placeholder_headers=$(curl -fsSI -H "Host: $request_host" "$edge_url/assets/cn-provider-shell-v10/zero-one-cn-provider-route-placeholder-v1.js")
