...
5.6.0.0/16`,
  ...
  usage: {
    ...
    accountCostPending: "缺少成本证据",
    accountCostUnsupportedScope: "暂不支持该计费范围",
    accountCostMissingEvidence: "历史记录/请求发生时未保存上游证据",
    accountCostUnknownSummary: "暂不支持 {unsupported} 条，缺少证据 {missing} 条",
    hideAccountCost: "隐藏账号成本",
    showAccountCost: "显示账号成本",
    accountCostStatus: "账号成本状态",
    accountCostConfirmed: "已确认",
  ...
  channelMonitorV2: {
    ...
    bootstrap: {
      title: "正在补齐历史监控数据",
      description: "首次启用被动监控时，系统会在后台静默聚合 90 分钟、24 小时、7 天与 30 天窗口；完成后可切换全部时间范围。",
      progress: "进度 {percent}%",
      working: "后台聚合中…"
    },
    timeRange: "时间范围",
    ...
    matrix: {
      title: "可用性趋势",
      description: "每行是一种渠道组合，每个色块代表一个统计区间；悬停查看明细",
      wheelZoom: "在色块上滚轮放大（区间变窄、色块变宽）",
      wheelZoomX: "在色块上滚轮放大（区间变窄、色块变宽）",
      dimension: "渠道维度",
      emptyTitle: "当前筛选窗口没有矩阵数据",
      legendAria: "健康分数图例",
      bad: "差",
      good: "好",
      healthyLegend: "健康 (≥80)",
      warningLegend: "需关注 (50–79)",
      criticalLegend: "异常 (<50)",
      unknownLegend: "无流量 / 样本不足",
      noTraffic: "该区间无流量",
      noTrafficAt: "{time} · 无流量",
      scoreLine: "健康分 {score}",
      resetZoom: "重置缩放"
    },
    chart: {
      title: "可用性趋势",
      description: "平滑趋势：错误率 · 首 Token P50 · 缓存率",
      emptyTitle: "当前筛选窗口没有趋势数据",
      errorLegend: "错误率（左轴 %）",
      cacheLegend: "缓存率（左轴 %）",
      ttftLegend: "首 Token P50（右轴）",
      errorDataset: "错误率趋势 %",
      cacheDataset: "缓存率趋势 %",
      ttftDataset: "首 Token 趋势 P50 (ms)",
      percentAxis: "比率 %",
      resetZoom: "重置缩放"
    },
    ...
    errorCategories: {
      ...
      timeout: "超时",
      transport_or_stream: "传输或流",
      upstream_forbidden: "上游拒绝",
      not_found: "资源不存在",
    ...
    settings: {
      title: "V2 数据监控配置",
      description: "配置被动用量汇总维度（平台 / 模型 / 分组）与刷新频率。健康色与明细在用户端 /monitor 以比例、RPM/TPM 展示，不暴露绝对请求量。",
      save: "保存",
      loading: "加载中...",
      loadFailed: "V2 配置加载失败",
      saveSuccess: "V2 监控配置已保存",
      saveFailed: "V2 配置保存失败",
      modeBanner: "当前系统设置为 {mode}。V2 分钟聚合不会运行；此处配置可预先保存，切换到 {modeV2} 后立即生效。可在系统设置 → 功能开关调整。",
      modeClosed: "渠道监控已关闭",
      modeV1: "V1 主动探测",
      modeV2: "V2 被动监控",
      ...
      errorsTitle: "错误分类与忽略",
      errorsHint: "勾选「忽略」的类别不计入错误率与健康分，仍在错误原因列表中以灰色显示并标记忽略。未匹配的错误归入「其他」。",
...
敏感词2`,
  ...
  channelMonitor: {
    ...
    form: {
      ...
      probeBillingWarning: "主动探测会向上游发送真实的生成请求，可能产生计费。",
      probeDailyEstimate: "按当前间隔和 {models} 个模型估算：每日约 {requests} 次生成请求。",
      quotaNoGeneration: "仅配额模式只读取账号用量或余额，不发送 LLM 生成请求。",
      checkModeQuota: "配额",
...
https://user:pass{'@'}proxy.example.com:443`,
  ...
  redeem: {
    ...
    minAmount: "最低额度 ($)",
    maxAmount: "最高额度 ($)",
    invalidMysteryBoxRange: "盲盒额度须为 0.01 至 999999999999.99 的金额，最多两位小数，且最高额度不得小于最低额度。",
    invalidAmount: "福利额度须为 0.01 至 999999999999.99 的金额，最多两位小数。",
    invalidCount: "生成数量须为 1 至 100 的整数。",
    ...
    deleteUnusedIncomplete: "已删除 {count} 个未使用兑换码，其余清理未完成。请刷新列表后重试。",
...
• 邀请码将重新生成为系统随机码（已分发的旧邀请链接将失效）`,
    ...
    site: {
      ...
      navigation: {
        title: "导航栏显示设置",
        description: "控制内置入口是否显示，以及模型广场在控制台中的位置。",
        profile: "个人资料",
        profileHint: "控制侧边导航中的“个人资料”入口；页面路由和右上角账户菜单不受影响。",
        subscriptions: "我的订阅",
        subscriptionsHint: "控制侧边导航中的“我的订阅”和顶部订阅进度入口。",
        modelPlaza: "模型广场位置",
      ...
      headerNavigation: {
        title: "顶部导航入口",
        description: "添加一个或多个二维码入口；点击顶部按钮后弹出对应二维码。",
        itemLabel: "顶部入口 #{n}",
        name: "顶部按钮及弹窗标题",
        namePlaceholder: "如：交流群",
        visibility: "可见范围",
        dialogDescription: "弹窗副标题",
        dialogDescriptionPlaceholder: "扫码加入交流群获取支持",
        qrImage: "二维码图片",
        selectQr: "选择二维码",
        removeQr: "移除",
        qrHint: "PNG、JPEG 或 WebP，最大 300 KiB。图片仅向登录用户按需提供。",
        qrRequired: "请先为新增的顶部导航入口选择二维码图片。",
    ...
    customMenu: {
      title: "自定义菜单页面",
      description: "添加自定义 iframe 页面到侧边栏、顶部导航或同时显示。每个页面可以设置显示位置及普通用户、管理员或全部登录用户可见。",
      itemLabel: "菜单项 #{n}",
      name: "菜单名称",
      ...
      iconPreview: "图标预览",
      iconChoices: "内置 SVG 图标",
      iconHint: "可选用上方 5 个内置 SVG，也可以上传自定义 SVG。",
      iconPresets: {
        link: "链接",
        users: "社群",
        gift: "礼物",
...
model not supported`,
  ...
  promptAudit: {
    ...
    events: {
      ...
      tabs: { summary: "审计摘要", risks: "具体风险", technical: "技术信息" },
...
您可以随时在右上角头像菜单中重新开启。`,
    ...
    user: {
      welcome: {
        title: "👋 欢迎使用零一 API",
        description: '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎使用零一 API。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: "开始 🚀",
        prevBtn: "跳过"
      },
      keyManage: {
