...
function financeCaption(scope) {
  if (scope.state === 'empty') return '账号成本 $0.00'
...
function renderSummary(today, total) {
  ...
  cards.querySelectorAll('.dashboard-finance-caption').forEach((node, index) => {
    node.textContent = index % 2 ? financeCaption(scopes[index]) : '客户实际扣费'
  })
}
...
function mount() {
  ...
  if (!trends?.isConnected) {
    ...
    details.append(element('p', 'dashboard-finance-caption', '收益＝同批账单的实际扣费－账号成本；成本按请求时的上游倍率计算。缺少历史倍率的账单不参与收益计算，旧账不会按当前倍率重算。'), table)
    trends.append(header, status, grid, details)
...
async function loadDays() {
  ...
  if (failed || values.today.charged === null || values.total.charged === null || authFailed) {
    ...
    status.textContent = !pending ? '点击或悬停曲线查看数值。'
      : values.total.state === 'unconfirmed' ? `${pending} 笔账单的成本与收益待确认。`
        : `收益为已核算部分，另有 ${pending} 笔待核算。`
  }
  controller = null
  if (authFailed && !authRecoveryRequested) {
