...
import { calendarDays, dailyValues, dateInTimezone, billedSummary, formatMoney, formatTokenConsumption } from './data.js'
...
function setFinanceLoading(loading) {
  ...
  retry.textContent = loading ? '读取中…' : '重新读取'
}
...
function financeCaption(scope) {
  if (scope.state === 'empty') return '当前范围没有已落库账单'
  if (scope.state === 'unconfirmed') return `${scope.unconfirmedRequests} 笔账单待核算`
  const prefix = scope.state === 'partial' ? '已核算成本' : '账号成本'
  const pending = scope.unconfirmedRequests ? ` · ${scope.unconfirmedRequests} 笔待核算` : ''
  return `${prefix} ${formatMoney(scope.confirmedCost)}${pending}`
}

function renderSummary(today, total) {
  const scopes = [today, today, total, total]
  const values = [today.charged, today.confirmedProfit, total.charged, total.confirmedProfit]
  const titles = [
    '今日消费', today.state === 'partial' ? '今日已核算收益' : '今日收益',
    '总消费', total.state === 'partial' ? '总计已核算收益' : '总收益',
  ]
  cards.querySelectorAll('.dashboard-finance-value').forEach((node, index) => {
    node.textContent = values[index] === null && index % 2 ? '待确认' : formatMoney(values[index])
    node.classList.toggle('is-negative', values[index] !== null && values[index] < 0)
  })
  cards.querySelectorAll('.dashboard-finance-label').forEach((node, index) => { node.textContent = titles[index] })
  cards.querySelectorAll('.dashboard-finance-caption').forEach((node, index) => {
    node.textContent = index % 2 ? financeCaption(scopes[index]) : '全部已落库账单的客户实际扣费'
  })
}
...
function drawCharts(rows) {
  ...
  charts = metrics.map((metric, index) => new Chart(trends.querySelectorAll('canvas')[index], {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: context => `${metric.title}：${metric.key === 'tokens' ? formatTokenConsumption(context.parsed.y) : formatMoney(context.parsed.y)}` } },
      },
      scales: {
        ...
        y: { beginAtZero: true, ticks: { color: dark ? '#a1a1aa' : '#6b7280', maxTicksLimit: 5, callback: value => metric.key === 'tokens' ? formatTokenConsumption(Number(value)) : formatMoney(Number(value)) }, grid: { color: dark ? '#ffffff12' : '#0000000c' } },
      },
    },
  }))
  ...
  body.replaceChildren(...rows.map(row => {
    const tr = element('tr')
    const state = row.state === 'failed' ? '读取失败'
      : row.state === 'complete' || row.state === 'empty' ? '已核算'
      : row.state === 'partial' ? `${row.confirmedRequests} 笔已核算 · ${row.unconfirmedRequests} 笔待核算`
        : `${row.unconfirmedRequests} 笔待核算`
    for (const value of [row.date, formatTokenConsumption(row.tokens), formatMoney(row.charged), row.cost === null ? '待确认' : formatMoney(row.cost), row.earnings === null ? '待确认' : formatMoney(row.earnings), state]) tr.append(element('td', '', value))
    return tr
  }))
}
...
function mount() {
  ...
  if (!cards?.isConnected) {
    ...
    for (const [index, title] of ['今日消费', '今日收益', '总消费', '总收益'].entries()) {
      ...
      card.append(element('p', 'dashboard-finance-label', title), element('strong', 'dashboard-finance-value', '—'), element('p', 'dashboard-finance-caption', index % 2 ? '等待成本核算' : '客户实际扣费'))
      cards.append(card)
    }
  ...
  if (!trends?.isConnected) {
    ...
    retry = element('button', 'btn btn-secondary', '重新读取')
    retry.type = 'button'
    retry.addEventListener('click', () => { if (!requestPending && !controller) loadDays() })
    ...
    for (const text of ['日期', '总 Token', '实际消费', '账号成本', '收益', '核算状态']) { const th = element('th', '', text); th.scope = 'col'; row.append(th) }
    head.append(row)
    table.append(head, element('tbody'))
    details.append(table)
    trends.append(header, status, grid, element('p', 'dashboard-finance-caption', '收益只在同一批已核算账单内按客户实际扣费减账号成本计算；成本使用请求采用的上游声明倍率。缺少依据的账单单独列为待核算，历史账单不会按当前倍率重算。'), details)
...
async function loadDays() {
  ...
  const rangeKey = `${current.start}:${current.end}:${current.timezone}`
  ...
  if (!days.length) { status.textContent = '日期范围无效，请重新选择。'; trends.setAttribute('aria-busy', 'false'); setFinanceLoading(false); return }
  const today = dateInTimezone(new Date(), current.timezone)
  const rows = days.map(date => ({ date, tokens: null, charged: null, cost: null, earnings: null, confirmedRequests: 0, unconfirmedRequests: 0, state: 'failed' }))
  let next = 0
  let complete = 0
  let failed = 0
  let todayStats = null
  ...
  if (failed && renderedRangeKey === rangeKey) {
    trends.setAttribute('aria-busy', 'false')
    setFinanceLoading(false)
    status.textContent = authFailed ? '登录状态失效或没有权限，请重新登录。' : '本次读取失败，当前仍显示上次成功结果。'
    ...
    } else if (!authFailed) scheduleRefresh()
    return
  }
  const values = billedSummary(todayStats, totalStats)
  renderSummary(values.today, values.total)
  cards.title = `今日：${today}（${current.timezone}）；总计：全部保留账单。`
  drawCharts(rows)
  renderedRangeKey = rangeKey
  trends.setAttribute('aria-busy', 'false')
  setFinanceLoading(false)
  if (failed || values.today.charged === null || values.total.charged === null || authFailed) {
    status.textContent = authFailed ? '登录状态失效或没有权限，请重新登录。' : '部分账单未能完整读取，缺失处已留空；请重新读取。'
    trends.querySelector('.dashboard-finance-updated').textContent = lastUpdated ? `本次更新未完成 · 上次完整更新 ${lastUpdated}` : '本次未完整更新'
  } else {
    lastUpdated = formatUpdatedAt(new Date(), current.timezone)
    trends.querySelector('.dashboard-finance-updated').textContent = `更新于 ${lastUpdated} · 页面可见时每 30 秒自动刷新`
    const pending = values.total.unconfirmedRequests
    status.textContent = !pending ? '金额来自已落库账单；点击或悬停曲线可查看数值。'
      : values.total.state === 'unconfirmed' ? `${pending} 笔账单缺少历史上游声明依据，成本与收益待确认。`
        : `已核算账单正常显示；另有 ${pending} 笔账单缺少历史上游声明依据，仍待核算。`
  }
  controller = null
  if (authFailed && !authRecoveryRequested) {
...
XMLHttpRequest.prototype.send = function(...args) {
  ...
  if (url && token) {
    ...
    this.addEventListener('loadend', () => {
      ...
      if (!calendarDays(start, end).length) { setFinanceLoading(false); return }
      identity = token
      snapshot = {
        start,
        end,
