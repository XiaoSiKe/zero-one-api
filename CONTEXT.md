...
管理员消费与收益 UI 对全部保留账单的实际扣费和依据历史上游声明倍率计算的账号成本进行展示；缺少有效声明依据的成本与收益标为待确认。收益是两者之差，保留负值，不用当前倍率重算历史，不等于充值流水或包含全部经营费用的净利润。可见页面约 30 秒刷新，显示完整更新时间及读取失败状态；今日与每日图使用相同 IANA 时区。实现、保留边界与日期浮层修复均为长期二开保护内容，见 [ADR 0010](docs/adr/0010-admin-billing-finance-and-date-panels.md)。
