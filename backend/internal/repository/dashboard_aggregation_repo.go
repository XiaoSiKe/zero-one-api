...
func (r *dashboardAggregationRepository) upsertHourlyAggregates(ctx context.Context, start, end time.Time) error {
	...
	query := `
		...
		INSERT INTO usage_dashboard_hourly (
			...
			actual_cost,
			account_cost,
			upstream_account_cost,
			upstream_cost_computed_at,
			total_duration_ms,
			active_users,
			computed_at
		)
		...
		DO UPDATE SET
			...
			upstream_account_cost = EXCLUDED.upstream_account_cost,
			upstream_cost_computed_at = EXCLUDED.upstream_cost_computed_at,
