<template>
    ...
    <!-- Shared create/edit group editor -->
    <GroupEditorDialog
      :show="showCreateModal || showEditModal"
      :mode="groupEditorMode"
      :title="t(groupEditorMode === 'edit' ? 'admin.groups.editGroup' : 'admin.groups.createGroup')"
      :ready="groupEditorMode === 'create' || Boolean(editingGroup)"
      :submitting="submitting"
      @close="closeGroupEditor"
      @submit="submitGroupEditor"
    >
        <div>
          <label class="input-label">{{ t("admin.groups.form.name") }}</label>
          <input
            v-model="activeGroupForm.name"
            type="text"
            required
            class="input"
            :placeholder="groupEditorMode === 'create' ? t('admin.groups.enterGroupName') : undefined"
            :data-tour="groupEditorMode === 'edit' ? 'edit-group-form-name' : 'group-form-name'"
          />
        </div>
        <div>
          ...
          <textarea
            v-model="activeGroupForm.description"
            rows="3"
            class="input"
            :placeholder="groupEditorMode === 'create' ? t('admin.groups.optionalDescription') : undefined"
          ></textarea>
        </div>
        <div>
          <label class="input-label">{{
          ...
          <Select
            v-model="activeGroupForm.platform"
            :options="platformOptions"
            :disabled="groupEditorMode === 'edit'"
            data-tour="group-form-platform"
            @change="handleGroupEditorPlatformChange"
          />
          <p class="input-hint">{{ t(groupEditorMode === "edit" ? "admin.groups.platformNotEditable" : "admin.groups.platformHint") }}</p>
        </div>
        <template v-if="!authStore.isSimpleMode">
        <!-- 从分组复制账号 -->
        <div v-if="activeCopyAccountsGroupOptions.length > 0">
              ...
              >
                ...
                >
                  <p class="text-xs leading-relaxed text-gray-300">
                    {{ t(groupEditorMode === "edit" ? "admin.groups.copyAccounts.tooltipEdit" : "admin.groups.copyAccounts.tooltip") }}
                  </p>
                  <div
          ...
          <!-- 已选分组标签 -->
          <div
            v-if="activeGroupForm.copy_accounts_from_group_ids.length > 0"
            class="flex flex-wrap gap-1.5 mb-2"
          >
            <span
              v-for="groupId in activeGroupForm.copy_accounts_from_group_ids"
              :key="groupId"
              class="inline-flex items-center gap-1 rounded-full bg-primary-100 px-2.5 py-1 text-xs font-medium text-primary-700 dark:bg-primary-900/30 dark:text-primary-300"
            >
              {{
                activeCopyAccountsGroupOptions.find((o) => o.value === groupId)
                  ?.label || `#${groupId}`
              }}
              <button
                type="button"
                @click="
                  activeGroupForm.copy_accounts_from_group_ids =
                    activeGroupForm.copy_accounts_from_group_ids.filter(
                      (id) => id !== groupId,
                    )
                "
          ...
          <select
            ...
            @change="
              (e) => {
                ...
                if (
                  val &&
                  !activeGroupForm.copy_accounts_from_group_ids.includes(val)
                ) {
                  activeGroupForm.copy_accounts_from_group_ids.push(val);
                }
          ...
          >
            ...
            <option
              v-for="opt in activeCopyAccountsGroupOptions"
              :key="opt.value"
              :value="opt.value"
              :disabled="
                activeGroupForm.copy_accounts_from_group_ids.includes(opt.value)
              "
            >
              {{ opt.label }}
            </option>
          </select>
          <p class="input-hint">{{ t(groupEditorMode === "edit" ? "admin.groups.copyAccounts.hintEdit" : "admin.groups.copyAccounts.hint") }}</p>
        </div>
        <div>
          <label class="input-label">{{
          ...
          <input
            v-model.number="activeGroupForm.rate_multiplier"
            type="number"
            step="0.001"
            min="0.001"
            required
          ...
          />
          <p v-if="groupEditorMode === 'create'" class="input-hint">{{ t("admin.groups.rateMultiplierHint") }}</p>
        </div>
        <div>
          <label class="input-label">{{ t("admin.groups.form.rpmLimit") }}</label>
          <input
            v-model.number="activeGroupForm.rpm_limit"
            type="number"
            min="0"
            step="1"
            class="input"
        ...
        <ReasoningEffortPolicyFields
          v-if="supportsReasoningEffortPolicyPlatform(activeGroupForm.platform)"
          :ref="setActiveReasoningEffortPolicyRef"
          id-prefix="create-group-reasoning"
          :platform="activeGroupForm.platform"
          v-model:max-effort="activeGroupForm.max_reasoning_effort"
          v-model:mappings="activeGroupForm.reasoning_effort_mappings"
        />
        <div
          v-if="activeGroupForm.subscription_type !== 'subscription'"
          :data-tour="groupEditorMode === 'create' ? 'group-form-exclusive' : undefined"
        >
          <div class="mb-1.5 flex items-center gap-1">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
          ...
          <div class="flex items-center gap-3">
            <button
              type="button"
              @click="activeGroupForm.is_exclusive = !activeGroupForm.is_exclusive"
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                activeGroupForm.is_exclusive
                  ? 'bg-primary-500'
            ...
            >
              <span
                :class="[
                  ...
                  activeGroupForm.is_exclusive ? 'translate-x-6' : 'translate-x-1',
                ]"
              />
            </button>
        ...
        <div v-if="groupEditorMode === 'edit'">
          <label class="input-label">{{ t("admin.groups.form.status") }}</label>
          <Select v-model="activeGroupForm.status" :options="editStatusOptions" />
        </div>
        ...
        <div class="mt-4 border-t pt-4">
          <div>
            ...
            <Select
              v-model="activeGroupForm.subscription_type"
              :options="subscriptionTypeOptions"
              :disabled="groupEditorMode === 'edit'"
            />
            <p class="input-hint">
              {{ t(groupEditorMode === "edit" ? "admin.groups.subscription.typeNotEditable" : "admin.groups.subscription.typeHint") }}
            </p>
          </div>
          ...
          >
            <div>
              ...
              <input
                v-model.number="activeGroupForm.daily_limit_usd"
                type="number"
                step="0.01"
                min="0"
                class="input"
            ...
            <div>
              ...
              <input
                v-model.number="activeGroupForm.weekly_limit_usd"
                type="number"
                step="0.01"
                min="0"
                class="input"
            ...
            <div>
              ...
              <input
                v-model.number="activeGroupForm.monthly_limit_usd"
                type="number"
                step="0.01"
                min="0"
                class="input"
        ...
        <div class="border-t pt-4">
          <div class="mb-3 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              @click="activeModelAllowlistState.enabled = !activeModelAllowlistState.enabled"
              :class="[
                'relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full transition-colors',
                activeModelAllowlistState.enabled
                  ? 'bg-primary-500'
                  : 'bg-gray-300 dark:bg-dark-600',
              ]"
            >
              <span
                :class="[
                  'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                  activeModelAllowlistState.enabled ? 'translate-x-6' : 'translate-x-1',
                ]"
              />
            </button>
          </div>
          <div
            v-if="activeModelAllowlistState.enabled"
            class="overflow-hidden rounded-lg border border-gray-200 bg-gray-50/50 dark:border-dark-600 dark:bg-dark-800/40"
          >
            <div
              v-if="!activeModelAllowlistLoading && activeModelAllowlistState.items.length > 0"
            ...
            >
              <span class="text-gray-500 dark:text-gray-400">
                {{
                  t("admin.groups.modelAllowlist.selectedSummary", {
                    selected: activeModelAllowlistSelectedCount,
                    total: activeModelAllowlistState.items.length,
                  })
                }}
              </span>
              <div class="flex items-center gap-1.5">
                <button
                  ...
                  @click="selectAllModelAllowlistItems(activeModelAllowlistState)"
                >
                ...
                <button
                  ...
                  @click="invertModelAllowlistSelection(activeModelAllowlistState)"
                >
            ...
            >
              <p v-if="activeModelAllowlistLoading" class="text-xs text-gray-500 dark:text-gray-400">
                {{ t("admin.groups.modelAllowlist.loading") }}
              </p>
              <p
                v-else-if="activeModelAllowlistState.items.length === 0"
                class="text-xs text-gray-500 dark:text-gray-400"
              >
                {{ t("admin.groups.modelAllowlist.empty") }}
              </p>
              <div
                v-for="(item, index) in activeModelAllowlistState.items"
                :key="item.id"
              ...
              >
                ...
                <button
                  ...
                  @click="moveGroupEditorModelAllowlistItem(index, index - 1)"
                >
                  <Icon name="arrowUp" size="sm" />
                </button>
                <button
                  type="button"
                  :disabled="index === activeModelAllowlistState.items.length - 1"
                  class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200"
                  @click="moveGroupEditorModelAllowlistItem(index, index + 1)"
                >
            ...
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              <div class="flex items-center gap-2">
                <input
                  v-model="activeAllowlistCustomEntry"
                  type="text"
                  ...
                  @keydown.enter.prevent="submitGroupEditorAllowlistCustomEntry"
                />
                <button
                  type="button"
                  class="rounded bg-primary-600 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-primary-700"
                  @click="submitGroupEditorAllowlistCustomEntry"
                >
                  {{ t("admin.groups.modelAllowlist.addCustom") }}
                </button>
              </div>
              <p v-if="activeAllowlistCustomErrorKey" class="mt-1 text-xs text-red-500">
                {{ t(activeAllowlistCustomErrorKey) }}
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- 图片生成计费配置 -->
        <div
          v-if="supportsImagePricingPlatform(activeGroupForm.platform)"
          class="border-t pt-4"
        >
          <label
            class="block mb-2 font-medium text-gray-700 dark:text-gray-300"
          >
            {{ t(imagePricingI18nKey(activeGroupForm.platform, "title")) }}
          </label>
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
            {{ t(imagePricingI18nKey(activeGroupForm.platform, "description")) }}
          </p>
          <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input
                v-model="activeGroupForm.allow_image_generation"
                type="checkbox"
                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              {{ t(imagePricingI18nKey(activeGroupForm.platform, "allowImageGeneration")) }}
            </label>
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input
                v-model="activeGroupForm.image_rate_independent"
                type="checkbox"
                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              {{ t(imagePricingI18nKey(activeGroupForm.platform, "independentMultiplier")) }}
            </label>
          </div>
          <div
            v-if="activeGroupForm.image_rate_independent"
            class="mb-4"
          >
            <label class="input-label">{{
              t(imagePricingI18nKey(activeGroupForm.platform, "imageMultiplier"))
            }}</label>
            <input
              v-model.number="activeGroupForm.image_rate_multiplier"
              type="number"
              step="0.0001"
              min="0"
              class="input"
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <input
                v-model.number="activeGroupForm.image_price_1k"
                type="number"
                step="0.001"
                min="0"
                class="input"
                :placeholder="getImagePricePlaceholder(activeGroupForm.platform, 'image_price_1k')"
              />
            </div>
            <div>
              <label class="input-label">2K ($)</label>
              <input
                v-model.number="activeGroupForm.image_price_2k"
                type="number"
                step="0.001"
                min="0"
                class="input"
                :placeholder="getImagePricePlaceholder(activeGroupForm.platform, 'image_price_2k')"
              />
            </div>
            <div>
              <label class="input-label">4K ($)</label>
              <input
                v-model.number="activeGroupForm.image_price_4k"
                type="number"
                step="0.001"
                min="0"
                class="input"
                :placeholder="getImagePricePlaceholder(activeGroupForm.platform, 'image_price_4k')"
              />
            </div>
          </div>
          <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">
            {{ t(imagePricingI18nKey(activeGroupForm.platform, "modeHint")) }}
          </p>
          <div class="mt-2 rounded-lg bg-gray-50 p-3 text-xs text-gray-700 dark:bg-gray-800 dark:text-gray-300">
            <div class="mb-1 font-medium">
              {{ t(imagePricingI18nKey(activeGroupForm.platform, "finalPricePreview")) }}
            </div>
            <div class="grid grid-cols-3 gap-2">
              <div
                v-for="item in activeImageFinalPricePreview"
                :key="item.label"
              >
          ...
          <div v-if="activeGroupForm.platform === 'gemini' && activeGroupForm.allow_image_generation" class="mt-4 border-t border-dashed border-gray-200 pt-4 dark:border-dark-700">
            <label
              class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300"
            >
              <input
                v-model="activeGroupForm.allow_batch_image_generation"
                type="checkbox"
                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
            ...
            <div
              v-if="activeGroupForm.allow_batch_image_generation"
            ...
            >
              <div>
                ...
                <input
                  v-model.number="activeGroupForm.batch_image_discount_multiplier"
                  type="number"
                  step="0.0001"
                  min="0"
                  class="input"
              ...
              <div>
                ...
                <input
                  v-model.number="activeGroupForm.batch_image_hold_multiplier"
                  type="number"
                  step="0.0001"
                  min="0"
                  class="input"
          ...
          <p
            v-else-if="activeGroupForm.platform !== 'gemini'"
        ...
        <!-- 视频生成计费配置（仅 Grok 平台） -->
        <div
          v-if="supportsVideoPricingPlatform(activeGroupForm.platform)"
          class="border-t pt-4"
        >
          <label
            class="block mb-2 font-medium text-gray-700 dark:text-gray-300"
          ...
          <div class="mb-4">
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input
                v-model="activeGroupForm.video_rate_independent"
                type="checkbox"
                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
          ...
          <div
            v-if="activeGroupForm.video_rate_independent"
            class="mb-4"
          >
            <label class="input-label">{{
            ...
            <input
              v-model.number="activeGroupForm.video_rate_multiplier"
              type="number"
              step="0.0001"
              min="0"
              class="input"
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <input
                v-model.number="activeGroupForm.video_price_480p"
                type="number"
                step="0.001"
                min="0"
                class="input"
                :placeholder="getVideoPricePlaceholder(activeGroupForm.platform, 'video_price_480p')"
              />
            </div>
            <div>
              <label class="input-label">720p ($/s)</label>
              <input
                v-model.number="activeGroupForm.video_price_720p"
                type="number"
                step="0.001"
                min="0"
                class="input"
                :placeholder="getVideoPricePlaceholder(activeGroupForm.platform, 'video_price_720p')"
              />
            </div>
            <div>
              <label class="input-label">1080p ($/s)</label>
              <input
                v-model.number="activeGroupForm.video_price_1080p"
                type="number"
                step="0.001"
                min="0"
                class="input"
                :placeholder="getVideoPricePlaceholder(activeGroupForm.platform, 'video_price_1080p')"
              />
            </div>
          </div>
          <div
          ...
          >
            ...
            <div class="mt-3 space-y-3">
              <div
                v-for="family in videoModelPriceFamilyRows(activeGroupForm.video_model_prices)"
                :key="family.key"
              ...
              >
                ...
                >
                  ...
                  <input
                    v-model.number="activeGroupForm.video_model_prices[family.key][resolution.key]"
                    type="number"
                    step="0.001"
                    min="0"
                    class="input"
          ...
          <div class="mt-2 rounded-lg bg-gray-50 p-3 text-xs text-gray-700 dark:bg-gray-800 dark:text-gray-300">
            ...
            <div class="grid grid-cols-3 gap-2">
              <div
                v-for="item in activeVideoFinalPricePreview"
                :key="item.label"
              >
                {{ item.label }}: {{ item.value }}
              </div>
        ...
        <!-- 高峰时段倍率配置（仅订阅类型分组） -->
        <div v-if="activeGroupForm.subscription_type === 'subscription'" class="border-t pt-4">
          <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input
                v-model="activeGroupForm.peak_rate_enabled"
                type="checkbox"
                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
          ...
          >
            ...
            <div>
              ...
              <input
                v-model="activeGroupForm.peak_end"
                type="time"
                class="input"
              />
            </div>
            <div>
              ...
              <input
                v-model.number="activeGroupForm.peak_rate_multiplier"
                type="number"
                step="0.001"
                min="0"
                class="input"
        ...
        <div v-if="isProfitControlPlatform(activeGroupForm.platform)" class="border-t pt-4">
          <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input
              v-model="activeGroupForm.profit_control_enabled"
              type="checkbox"
              class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
          ...
          >
            ...
            <div>
              ...
              <input
                v-model.number="activeGroupForm.profit_safety_buffer_percent"
                type="number"
                step="0.1"
                min="0"
                max="99.99"
        ...
        <div v-if="activeGroupForm.platform === 'antigravity'" class="border-t pt-4">
          <div class="mb-1.5 flex items-center gap-1">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
          ...
          <div class="space-y-2">
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                :checked="activeGroupForm.supported_model_scopes.includes('claude')"
                @change="toggleGroupEditorScope('claude')"
            ...
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                :checked="
                  activeGroupForm.supported_model_scopes.includes('gemini_text')
                "
                @change="toggleGroupEditorScope('gemini_text')"
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{
            ...
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                :checked="
                  activeGroupForm.supported_model_scopes.includes('gemini_image')
                "
                @change="toggleGroupEditorScope('gemini_image')"
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{
        ...
        <div v-if="activeGroupForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            <button
              type="button"
              @click="activeGroupForm.mcp_xml_inject = !activeGroupForm.mcp_xml_inject"
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                activeGroupForm.mcp_xml_inject
                  ? 'bg-primary-500'
                  : 'bg-gray-300 dark:bg-dark-600',
              ]"
            >
              <span
                :class="[
                  'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                  activeGroupForm.mcp_xml_inject ? 'translate-x-6' : 'translate-x-1',
                ]"
              />
            </button>
            <span class="text-sm text-gray-500 dark:text-gray-400">
        ...
        <div v-if="activeGroupForm.platform === 'anthropic'" class="border-t pt-4">
          <div class="mb-1.5 flex items-center gap-1">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
          ...
          <div class="flex items-center gap-3">
            <button
              type="button"
              @click="
                activeGroupForm.claude_code_only = !activeGroupForm.claude_code_only
              "
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                activeGroupForm.claude_code_only
                  ? 'bg-primary-500'
                  : 'bg-gray-300 dark:bg-dark-600',
              ]"
            >
          ...
          <div v-if="activeGroupForm.claude_code_only" class="mt-3">
            <label class="input-label">{{
              t("admin.groups.claudeCode.fallbackGroup")
            }}</label>
            <Select
              v-model="activeGroupForm.fallback_group_id"
              :options="activeFallbackGroupOptions"
        ...
        <div
          v-if="activeGroupForm.platform === 'openai'"
        ...
        >
          ...
          <div>
            ...
            <input
              v-model.number="activeGroupForm.web_search_price_per_call"
              type="number"
              step="0.001"
              min="0"
              placeholder="0.01"
            ...
            >
              {{
                t("admin.groups.webSearchPricing.finalPricePreview", {
                  price: activeWebSearchFinalPricePreview,
                })
              }}
            </div>
          </div>
        ...
        <div class="border-t border-gray-200 pt-4 mt-4 dark:border-dark-400">
          <div class="flex items-start justify-between gap-4">
            ...
            <button type="button" class="btn btn-secondary" @click="addGroupPricing(activeGroupForm.model_pricing)">
              <Icon name="plus" size="sm" class="mr-1" />{{ t("admin.groups.modelPricing.add") }}
            </button>
          </div>
          <label class="mt-3 flex items-start gap-2">
            <input v-model="activeGroupForm.long_context_pricing_enabled" type="checkbox" class="mt-0.5" />
          ...
          <div class="mt-3 space-y-2">
            <PricingEntryCard v-for="(entry, index) in activeGroupForm.model_pricing" :key="index" :entry="entry" :platform="activeGroupForm.platform" hide-token-intervals @update="activeGroupForm.model_pricing[index] = $event" @remove="activeGroupForm.model_pricing.splice(index, 1)" />
          </div>
        </div>

        <!-- Grok Voice 显式定价（仅 grok 平台） -->
        <div
          v-if="activeGroupForm.platform === 'grok'"
          class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4"
        >
          ...
          <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
            <div>
              ...
              <input
                v-model.number="activeGroupForm.search_price_per_1k"
                type="number"
                step="0.000001"
                min="0"
                class="input"
            ...
            <div>
              ...
              <input
                v-model.number="activeGroupForm.audio_realtime_price_per_min"
                type="number"
                step="0.000001"
                min="0"
                class="input"
            ...
            <div>
              ...
              <input
                v-model.number="activeGroupForm.audio_tts_price_per_million_chars"
                type="number"
                step="0.000001"
                min="0"
                class="input"
            ...
            <div>
              ...
              <input
                v-model.number="activeGroupForm.audio_stt_price_per_hour"
                type="number"
                step="0.000001"
                min="0"
                class="input"
          ...
          <div class="flex items-center justify-between">
            ...
            >
              <span
                ...
                :class="activeGroupForm.allow_live ? 'translate-x-6' : 'translate-x-1'"
              />
            </button>
          </div>
        ...
        >
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              type="button"
              @click="
                activeGroupForm.allow_messages_dispatch =
                  !activeGroupForm.allow_messages_dispatch
              "
              class="relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
              :class="
          ...
          <div v-if="activeGroupForm.allow_messages_dispatch" class="mt-3">
            <div
              ...
              <div class="p-4">
                <div class="grid gap-4 md:grid-cols-3">
                  <div>
                    ...
                    <input
                      v-model="activeGroupForm.opus_mapped_model"
                      type="text"
                      :placeholder="
                  ...
                  <div>
                    ...
                    <input
                      v-model="activeGroupForm.sonnet_mapped_model"
                      type="text"
                      :placeholder="
                  ...
                  <div>
                    ...
                    <input
                      v-model="activeGroupForm.haiku_mapped_model"
                      type="text"
                      :placeholder="
            ...
            >
              ...
              <div class="p-4 bg-gray-50/30 dark:bg-dark-800/30">
                <div
                  v-if="activeGroupForm.exact_model_mappings.length === 0"
                ...
                >
                  ...
                  <button
                    type="button"
                    @click="addGroupEditorMessagesDispatchMapping"
                ...
                <div v-else class="space-y-3">
                  <div
                    v-for="row in activeGroupForm.exact_model_mappings"
                    :key="getGroupEditorMessagesDispatchRowKey(row)"
                  ...
                  >
                    <div class="flex items-center gap-4">
                      ...
                      <button
                        type="button"
                        @click="removeGroupEditorMessagesDispatchMapping(row)"
        ...
        >
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{
                  activeGroupForm.require_oauth_only
            ...
            <button
              type="button"
              @click="
                activeGroupForm.require_oauth_only = !activeGroupForm.require_oauth_only
              "
              class="relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
              :class="
                activeGroupForm.require_oauth_only
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{
                  activeGroupForm.require_privacy_set
            ...
            <button
              type="button"
              @click="
                activeGroupForm.require_privacy_set = !activeGroupForm.require_privacy_set
              "
              class="relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
              :class="
                activeGroupForm.require_privacy_set
        ...
        <div
          v-if="
            ['anthropic', 'antigravity'].includes(activeGroupForm.platform) &&
            activeGroupForm.subscription_type !== 'subscription'
          "
          class="border-t pt-4"
        >
          <label class="input-label">{{
          ...
          <Select
            v-model="activeGroupForm.fallback_group_id_on_invalid_request"
            :options="activeInvalidRequestFallbackOptions"
        ...
        <div v-if="activeGroupForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3 mb-3">
            <button
              type="button"
              @click="
                activeGroupForm.model_routing_enabled =
                  !activeGroupForm.model_routing_enabled
              "
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
          ...
          <p
            v-if="!activeGroupForm.model_routing_enabled"
          ...
          <!-- 路由规则列表（仅在启用时显示） -->
          <div v-if="activeGroupForm.model_routing_enabled" class="space-y-3">
            <div
              v-for="rule in activeModelRoutingRules"
              :key="getGroupEditorRuleRenderKey(rule)"
            ...
            >
              <div class="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          type="button"
                          @click="removeSelectedAccount(rule, account.id, groupEditorMode === 'edit')"
                          class="ml-0.5 text-primary-500 hover:text-primary-700 dark:hover:text-primary-200"
                        >
                          <Icon name="x" size="xs" />
                        </button>
                    ...
                    <div class="relative account-search-container">
                      <input
                        v-model="
                          accountSearchKeyword[getGroupEditorRuleSearchKey(rule)]
                        "
                        type="text"
                        class="input text-sm"
                        :placeholder="
                        ...
                        "
                        @input="searchAccountsByRule(rule, groupEditorMode === 'edit')"
                        @focus="onAccountSearchFocus(rule, groupEditorMode === 'edit')"
                      />
                      <!-- 搜索结果下拉框 -->
                      <div
                        v-if="
                          showAccountDropdown[getGroupEditorRuleSearchKey(rule)] &&
                          accountSearchResults[getGroupEditorRuleSearchKey(rule)]
                            ?.length > 0
                        "
                      ...
                      >
                        <button
                          v-for="account in accountSearchResults[
                            getGroupEditorRuleSearchKey(rule)
                          ]"
                          :key="account.id"
                          type="button"
                          @click="selectAccount(rule, account, groupEditorMode === 'edit')"
                ...
                <button
                  type="button"
                  @click="removeGroupEditorRoutingRule(rule)"
          ...
          <button
            v-if="activeGroupForm.model_routing_enabled"
            type="button"
            @click="addGroupEditorRoutingRule"
...
import GroupEditorDialog from "@/components/admin/group/GroupEditorDialog.vue";
...
const platformOptions = computed(() =>
  GROUP_PLATFORM_OPTIONS.filter((option) => !authStore.isSimpleMode || option.value !== "composite"),
);
...
const addCustomAllowlistEntry = (
  state: typeof createModelAllowlistState,
  entry: typeof createAllowlistCustomEntry,
  errorKey: typeof createAllowlistCustomErrorKey,
) => {
  const error = addCustomModelAllowlistItem(state, entry.value);
  if (error) {
    errorKey.value = `admin.groups.modelAllowlist.errors.${error}`;
    return;
  }
  entry.value = "";
  errorKey.value = null;
...
const submitCreateAllowlistCustomEntry = () =>
  addCustomAllowlistEntry(createModelAllowlistState, createAllowlistCustomEntry, createAllowlistCustomErrorKey);
const submitEditAllowlistCustomEntry = () =>
  addCustomAllowlistEntry(editModelAllowlistState, editAllowlistCustomEntry, editAllowlistCustomErrorKey);
...
// One editor surface owns the shared create/edit fields. The two form objects
// remain separate because their submit payloads and reset lifecycles differ.
const groupEditorMode = computed<"create" | "edit">(() =>
  showEditModal.value ? "edit" : "create",
);
const activeGroupForm = computed(() =>
  groupEditorMode.value === "edit" ? editForm : createForm,
);
const activeCopyAccountsGroupOptions = computed(() =>
  groupEditorMode.value === "edit"
    ? copyAccountsGroupOptionsForEdit.value
    : copyAccountsGroupOptions.value,
);
const activeFallbackGroupOptions = computed(() =>
  groupEditorMode.value === "edit"
    ? fallbackGroupOptionsForEdit.value
    : fallbackGroupOptions.value,
);
const activeInvalidRequestFallbackOptions = computed(() =>
  groupEditorMode.value === "edit"
    ? invalidRequestFallbackOptionsForEdit.value
    : invalidRequestFallbackOptions.value,
);
const activeModelAllowlistState = computed(() =>
  groupEditorMode.value === "edit"
    ? editModelAllowlistState
    : createModelAllowlistState,
);
const activeModelAllowlistLoading = computed(() =>
  groupEditorMode.value === "edit"
    ? editModelAllowlistLoading.value
    : createModelAllowlistLoading.value,
);
const activeModelAllowlistSelectedCount = computed(() =>
  groupEditorMode.value === "edit"
    ? editModelAllowlistSelectedCount.value
    : createModelAllowlistSelectedCount.value,
);
const activeAllowlistCustomEntry = computed({
  get: () => groupEditorMode.value === "edit"
    ? editAllowlistCustomEntry.value
    : createAllowlistCustomEntry.value,
  set: (value: string) => {
    if (groupEditorMode.value === "edit") editAllowlistCustomEntry.value = value;
    else createAllowlistCustomEntry.value = value;
  },
});
const activeAllowlistCustomErrorKey = computed(() =>
  groupEditorMode.value === "edit"
    ? editAllowlistCustomErrorKey.value
    : createAllowlistCustomErrorKey.value,
);
const activeImageFinalPricePreview = computed(() =>
  groupEditorMode.value === "edit"
    ? editImageFinalPricePreview.value
    : createImageFinalPricePreview.value,
);
const activeVideoFinalPricePreview = computed(() =>
  groupEditorMode.value === "edit"
    ? editVideoFinalPricePreview.value
    : createVideoFinalPricePreview.value,
);
const activeWebSearchFinalPricePreview = computed(() =>
  groupEditorMode.value === "edit"
    ? editWebSearchFinalPricePreview.value
    : createWebSearchFinalPricePreview.value,
);
const activeModelRoutingRules = computed(() =>
  groupEditorMode.value === "edit"
    ? editModelRoutingRules.value
    : createModelRoutingRules.value,
);

const setActiveReasoningEffortPolicyRef = (
  instance: unknown,
) => {
  const policyFields = instance as ReasoningEffortPolicyFieldsExpose | null;
  if (groupEditorMode.value === "edit") editReasoningEffortPolicyRef.value = policyFields;
  else createReasoningEffortPolicyRef.value = policyFields;
};
const submitGroupEditorAllowlistCustomEntry = () =>
  groupEditorMode.value === "edit"
    ? submitEditAllowlistCustomEntry()
    : submitCreateAllowlistCustomEntry();
const moveGroupEditorModelAllowlistItem = (fromIndex: number, toIndex: number) =>
  groupEditorMode.value === "edit"
    ? moveEditModelAllowlistItem(fromIndex, toIndex)
    : moveCreateModelAllowlistItem(fromIndex, toIndex);
const toggleGroupEditorScope = (scope: string) =>
  groupEditorMode.value === "edit" ? toggleEditScope(scope) : toggleCreateScope(scope);
const addGroupEditorMessagesDispatchMapping = () =>
  groupEditorMode.value === "edit"
    ? addEditMessagesDispatchMapping()
    : addCreateMessagesDispatchMapping();
const removeGroupEditorMessagesDispatchMapping = (row: MessagesDispatchMappingRow) =>
  groupEditorMode.value === "edit"
    ? removeEditMessagesDispatchMapping(row)
    : removeCreateMessagesDispatchMapping(row);
const getGroupEditorMessagesDispatchRowKey = (row: MessagesDispatchMappingRow) =>
  groupEditorMode.value === "edit"
    ? getEditMessagesDispatchRowKey(row)
    : getCreateMessagesDispatchRowKey(row);
const getGroupEditorRuleRenderKey = (rule: ModelRoutingRule) =>
  groupEditorMode.value === "edit"
    ? getEditRuleRenderKey(rule)
    : getCreateRuleRenderKey(rule);
const getGroupEditorRuleSearchKey = (rule: ModelRoutingRule) =>
  groupEditorMode.value === "edit"
    ? getEditRuleSearchKey(rule)
    : getCreateRuleSearchKey(rule);
const addGroupEditorRoutingRule = () =>
  groupEditorMode.value === "edit" ? addEditRoutingRule() : addCreateRoutingRule();
const removeGroupEditorRoutingRule = (rule: ModelRoutingRule) =>
  groupEditorMode.value === "edit"
    ? removeEditRoutingRule(rule)
    : removeCreateRoutingRule(rule);
const closeGroupEditor = () =>
  groupEditorMode.value === "edit" ? closeEditModal() : closeCreateModal();
const submitGroupEditor = () =>
  groupEditorMode.value === "edit" ? handleUpdateGroup() : handleCreateGroup();
const handleGroupEditorPlatformChange = () => {
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    const payload = authStore.isSimpleMode
      ? { name: createForm.name, description: createForm.description, platform: createForm.platform }
      : requestData;
